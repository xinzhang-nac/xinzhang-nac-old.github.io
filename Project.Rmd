...
**Consensus Learning in Network System**  [Dec 2017 – Dec 2020]{style="float:right"}   
-- Advisor: Dr. Jia Liu, (CS Dept.@ISU)  

* Decentralized learning algorithms work with the computational system formed by a network of nodes, each of which can locally store the data and pass the message through the network links. The goal is to train a global learning model based on all nodes’ data through local computation and neighboring communication. Current decentralized learning methods suffer from 1) high computational complexity due to the large local datasets; 2) inefficient communication caused by the low network bandwidth and complex model structure; 3) significant loss of data privacy when unreliable nodes exist. My work focuses on designing decentralized algorithms that can overcome these problems.       
